...
public sealed class PointerFieldRecoveryStrategy : IRecoveryStrategy
{
        ...
        RecoveryContext context)
    {
        ...
        for (var off = startOffset; off <= endOffset; off += step)
        {
            ...
            if (ptr == IntPtr.Zero || !reader.IsValidAddress(ptr))
            {
                continue;
            }
            ...
            candidate.Evidence.Add(new EvidenceRecord
            {
                ...
                Description = $"Pointer 0x{ptr.ToInt64():X} is in valid memory range",
                ScoreDelta = 25,
                Passed = true,
            ...
            if (childNode != null)
            {
                ...
                {
                    if (reader.TryRead<IntPtr>(ptr + childOffset, out var childPtr) && reader.IsValidAddress(childPtr))
                    {
                        candidate.Evidence.Add(new EvidenceRecord
                        {
                ...
                else if (childNode.Kind == ValueKind.StdVectorField)
                {
                    ...
                    {
                        if (reader.IsValidAddress(b) && reader.IsValidAddress(e) && b.ToInt64() <= e.ToInt64() && (c == IntPtr.Zero || e.ToInt64() <= c.ToInt64()))
                        {
                            candidate.Evidence.Add(new EvidenceRecord
                            {
