...
- **DO NOT advance/bump versions and DO NOT update CHANGELOG.md** during routine development, iterations, or bug fixes.
- Only update versions in `TEHhub.csproj` / `TEHhub.Launcher.csproj` and append entries to `CHANGELOG.md` when the user explicitly gives the command to create or publish a **Release**.
- When instructed to release: keep core and launcher versions synchronized; assembly/file versions use x.x.x.0, and explicitly label whether the release is an **[SDK / Core Framework Update]** or **[Plugin Update — PluginName]** in `CHANGELOG.md` and commit messages.
...
## 5. PoE 2 Expedition Mechanics & Calculation Reference

### Core Units & Rounding Rules
- **PoE Engine Rounding Rule**: Integer counts, charges, and stats are **ALWAYS FLOORED** (`Math.Floor` / ปัดเศษลงเสมอ).
- **World to Grid Conversion**: `1 Grid ≈ 10.87 World` (`250 / 23`). `1.0 metre = 100 World Units ≈ 9.2 Grid`.

### Encounter Types & Base Parameters
| Encounter Type | Base Explosives | Explosion Radius | Placement Reach (Max Wire) |
| :--- | :---: | :---: | :---: |
| **Regular Map Encounter** | **5** | **300 W** (3.0m, ~27.6 Grid) | **1,000 W** (10.0m, ~92.0 Grid) |
| **Grand Expedition** | **15** | **380 W** (3.8m, ~35.0 Grid) | **1,200 W** (12.0m, ~110.4 Grid) |

### Dynamic Map Mod Formula
- `TotalExplosives = Floor(BaseCount * (1.0 + (IncreasedExplosivesPct / 100.0)))`
  - Example: Base 5 with `42% increased number of Expedition Explosives`: `Floor(5 * 1.42) = Floor(7.1) = 7`.
- `TotalRadius = BaseRadius * (1.0 + (IncreasedRadiusPct / 100.0))`
- SDK API: `Core.States.InGameStateObject.CurrentAreaInstance.ExpeditionConfig` or `TEHhub.Utils.ExpeditionMechanics.GetConfig(area)`.
