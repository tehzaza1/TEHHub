...

{
    internal static void Run(Action<bool, string> check)
    {
        ...
        // =========================================================================
        // AmanamuVoidAlert Candidate Filtering Unit Tests
        // =========================================================================
        // 1. Path prefilter validation: rejects ordinary monsters, accepts Abyss/Amanamu
        check(!AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidatePath(null), "IsCandidatePath rejects null");
        check(!AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidatePath(string.Empty), "IsCandidatePath rejects empty path");
        check(!AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidatePath("Metadata/Monsters/Zombies/ZombieStandard"), "IsCandidatePath rejects standard zombie");
        check(!AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidatePath("Metadata/Monsters/Spiders/SpiderPlague"), "IsCandidatePath rejects ordinary spider");
        check(!AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidatePath("Metadata/Monsters/Skeletons/SkeletonMelee"), "IsCandidatePath rejects ordinary skeleton");
        check(!AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidatePath("Metadata/Monsters/Daemon/InvisibleMonster"), "IsCandidatePath rejects ordinary daemon");
        check(AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidatePath("Metadata/Monsters/LeagueAbyss/AbyssLichAmanamu"), "IsCandidatePath accepts Amanamu lich");
        check(AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidatePath("Metadata/Monsters/LeagueAbyss/LightlessLeader"), "IsCandidatePath accepts LightlessLeader");
        check(AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidatePath("Metadata/Monsters/LeagueAbyss/AbyssLichUlaman"), "IsCandidatePath accepts AbyssLich");
        check(AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidatePath("Metadata/Monsters/LeagueAbyss/AbyssMonster1"), "IsCandidatePath accepts LeagueAbyss path");

        // 2. Mod prefilter validation: rejects ordinary mods, accepts Abyss Lightless mods
        check(!AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidateMod(null), "IsCandidateMod rejects null");
        check(!AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidateMod(string.Empty), "IsCandidateMod rejects empty mod");
        check(!AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidateMod("MonsterExtraDamage"), "IsCandidateMod rejects ordinary extra damage mod");
        check(!AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidateMod("MonsterResistFire"), "IsCandidateMod rejects ordinary resistance mod");
        check(!AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidateMod("MonsterFastMovement"), "IsCandidateMod rejects ordinary speed mod");
        check(AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidateMod("MonsterAbyssLightlessFaction1"), "IsCandidateMod accepts MonsterAbyssLightlessFaction1");
        check(AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidateMod("Metadata/Monsters/MonsterMods/LeagueAbyss/LightlessWells"), "IsCandidateMod accepts LightlessWells metadata mod");
        check(AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidateMod("MonsterAbyssLightlessBoss"), "IsCandidateMod accepts MonsterAbyssLightless prefix");

        // 3. Composite candidate predicate tests: ordinary monster does not qualify
        {
            var ordinaryPath = "Metadata/Monsters/Zombies/ZombieStandard";
            var ordinaryMod = "MonsterExtraDamage";
            var isCandidatePath = AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidatePath(ordinaryPath);
            var isCandidateMod = AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidateMod(ordinaryMod);
            var alreadyKnown = false;
            var isCandidate = alreadyKnown || isCandidatePath || isCandidateMod;
            check(!isCandidate, "Composite candidate predicate rejects ordinary nearby monster before Buffs request");
        }

        // 4. Composite candidate predicate tests: Amanamu by path qualifies
        {
            var amanamuPath = "Metadata/Monsters/LeagueAbyss/AbyssLichAmanamu";
            var isCandidatePath = AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidatePath(amanamuPath);
            var isCandidateMod = AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidateMod("MonsterExtraDamage");
            var alreadyKnown = false;
            var isCandidate = alreadyKnown || isCandidatePath || isCandidateMod;
            check(isCandidate, "Composite candidate predicate accepts Amanamu path candidate");
        }

        // 5. Composite candidate predicate tests: Abyss monster by mod qualifies
        {
            var genericPath = "Metadata/Monsters/GenericMonster";
            var abyssMod = "MonsterAbyssLightlessFaction1";
            var isCandidatePath = AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidatePath(genericPath);
            var isCandidateMod = AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidateMod(abyssMod);
            var alreadyKnown = false;
            var isCandidate = alreadyKnown || isCandidatePath || isCandidateMod;
            check(isCandidate, "Composite candidate predicate accepts Abyss monster by mod");
        }

        // 6. Composite candidate predicate tests: Already-known monster qualifies
        {
            var ordinaryPath = "Metadata/Monsters/Zombies/ZombieStandard";
            var isCandidatePath = AmanamuVoidAlert.AmanamuVoidAlertCore.IsCandidatePath(ordinaryPath);
            var isCandidateMod = false;
            var alreadyKnown = true;
            var isCandidate = alreadyKnown || isCandidatePath || isCandidateMod;
            check(isCandidate, "Composite candidate predicate keeps already-known monster eligible for Buffs cloud status tracking");
        }

        // 7. Alert state evaluation logic verification
        {
            const string BuffPrefixAbyssLightlessWell = "abyss_lightless_well";
            const string BuffInsideCloud = "abyss_lightless_well_immune";

            var sampleBuffsInCloud = new[] { "abyss_lightless_well", "abyss_lightless_well_immune", "quick" };
            var insideCloud = sampleBuffsInCloud.Any(b => b.Contains(BuffInsideCloud, StringComparison.OrdinalIgnoreCase));
            var hasAnyLightlessBuff = sampleBuffsInCloud.Any(b => b.Contains(BuffPrefixAbyssLightlessWell, StringComparison.OrdinalIgnoreCase));
            check(insideCloud, "Alert state correctly identifies inside-cloud status from immunity buff");
            check(hasAnyLightlessBuff, "Alert state correctly identifies lightless well presence");

            var sampleBuffsOutsideCloud = new[] { "abyss_lightless_well", "quick" };
            var outsideCloud = !sampleBuffsOutsideCloud.Any(b => b.Contains(BuffInsideCloud, StringComparison.OrdinalIgnoreCase));
            check(outsideCloud, "Alert state correctly identifies outside-cloud status when immunity buff is absent");
        }
    }
