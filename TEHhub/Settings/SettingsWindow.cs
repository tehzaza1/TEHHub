...
namespace TEHhub.Settings
{
    ...
    internal static class SettingsWindow
    {
        ...
        private static void DrawManuBar()
        {
            ...
            ImGui.Text($"TEHhub {Core.GetVersion()}");
            ImGui.PopStyleColor();
            ImGui.SameLine();
            ImGui.TextDisabled("|");
            ImGui.SameLine();
        ...
        private static void DrawUiLanguageWidget()
        {
            ...
            ImGuiHelper.ToolTip(
                L.T(
                    ...
                    "Controls TEHhub's main overlay text. Font glyph range is configured separately below."));
        }
            ...
            if (ImGui.CollapsingHeader(L.Title("settings.input.title", "Input Config", "InputConfig")))
            {
                ...
                ImGuiHelper.ToolTip(
                    L.T(
                        ...
                        "When TEHhubOverlay press a key in the game, the key has to go to the GGG server for it to work. This process takes time equal to your latency x 3. During this time TEHhubOverlay might press that key again. Set the key timeout value to latency x 3 so this doesn't happen. e.g. for 30ms latency, set it to 90ms. Also, do not go below 60 (due to server ticks), no matter how good your latency is."));
        ...
        private static void DrawToolsConfig()
        {
            if (ImGui.CollapsingHeader(L.Title("settings.tools.title", "Misc Tools", "MiscTools")))
            {
                ...
                ImGui.Checkbox(L.Label("settings.tools.offset_helper_v2", "OffsetHelper V2 (Dev/Practical)", "OffsetHelperV2"), ref Core.GHSettings.ShowOffsetHelperV2);
#if DEBUG
        ...
        private static void DrawMiscConfig()
        {
            if (ImGui.CollapsingHeader(L.Title("settings.misc.title", "Miscellaneous Config", "MiscellaneousConfig")))
            {
                if (ImGui.Checkbox(L.Label("settings.misc.fix_taskbar", "Fix Taskbar not showing", "FixTaskbarNotShowing"), ref Core.GHSettings.FixTaskbarNotShowing))
                {
                    ...
                    {
                        CoroutineHandler.RaiseEvent(TEHhubEvents.OnMoved);
                    }
                }
                ...
                ImGui.Checkbox(L.Label("settings.misc.close_when_game_exit", "Close TEHhub when Game Exit", "CloseWhenGameExit"), ref Core.GHSettings.CloseWhenGameExit);
        ...
        private static void DrawConfirmationPopup()
        {
            ...
            if (ImGui.BeginPopup("TEHhubCloseConfirmation"))
            {
                ImGui.Text(L.T("settings.confirm.quit", "Do you want to quit the TEHhub overlay?"));
                ImGui.Separator();
        ...
        private static IEnumerator<Wait> RenderCoroutine()
        {
            while (true)
            {
                ...
                {
                    ImGui.OpenPopup("TEHhubCloseConfirmation");
                }
        ...
        /// <summary>
        ///     Saves the TEHhub settings to disk.
        /// </summary>
        /// <returns>co-routine IWait.</returns>
        private static IEnumerator<Wait> SaveCoroutine()
        {
            while (true)
            {
                yield return new Wait(TEHhubEvents.TimeToSaveAllSettings);
