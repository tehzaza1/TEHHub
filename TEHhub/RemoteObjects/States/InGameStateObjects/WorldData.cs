...
namespace TEHhub.RemoteObjects.States.InGameStateObjects
{
    ...
    using TEHhub.RemoteObjects.FilesStructures;
    ...
    public class WorldData : RemoteObjectBase
    {
        ...
        private IEnumerator<Wait> OnPerFrame()
        {
            while (true)
            {
                yield return new Wait(TEHhubEvents.PostPerFrameDataUpdate);
                if (this.Address != IntPtr.Zero)
                {
                    using var memoryReadRegion = TEHhub.Ui.MemoryReadDiagnostics.MeasureRegion("Core.WorldData");
