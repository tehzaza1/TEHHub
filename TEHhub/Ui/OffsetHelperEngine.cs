...
namespace TEHhub.Ui
{
    ...
    ///     <c>[StructLayout(LayoutKind.Explicit)]</c> offset structs in <c>TEHhub.Offsets</c> via
    ...
    internal static class OffsetHelperEngine
    {
        ...
        ///     class name) to the <c>TEHhub.Offsets</c> struct that describes its byte layout. Only
        ...
        internal static readonly Dictionary<string, Type> ComponentOffsetTypes = new(StringComparer.Ordinal)
        {
            ...
            ["DiesAfterTime"] = typeof(ComponentHeader),
            ["Life"] = typeof(LifeOffset),
            ["MinimapIcon"] = typeof(MinimapIconOffsets),
            ["Mods"] = typeof(ModsOffsets),
            ["Monster"] = typeof(ComponentHeader),
            ["NPC"] = typeof(ComponentHeader),
            ...
            OffsetRecoveryHints? hints)
        {
            ...
            for (var i = 0; i < take; i++)
            {
                ...
                var verifiedRoot = VerifyRoot(probeType, roots[i].Label, roots[i].Address, roots[i].Owner, ownerAnchorField, rootHints);
                verifiedRoot.UseRecoveryHints = roots[i].UseRecoveryHints;
                res.Roots.Add(verifiedRoot);
            }
        ...
        /// </summary>
        internal static List<OffsetRecoverySuggestion> FindOffsetRecoveries(
            Type structType,
            List<Root> roots,
            string ownerAnchorField,
            OffsetRecoveryHints? hints)
    ...
    {
        public bool UseRecoveryHints { get; set; }
