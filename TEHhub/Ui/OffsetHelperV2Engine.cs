    ...
    public static class OffsetHelperV2Engine
    {
        ...
        private static V2ProbeResult ProbeLocalPlayer(SafeMemoryHandle reader)
        {
            ...
            try
            {
                ...
                var compVec = CanonicalStructuralInvariants.ValidateStdVector(entity.ItemBase.ComponentListPtr, elementSize: 8);
                details["ComponentListVector"] = compVec.Detail;

                if (!compVec.IsValid)
                {
                    return new V2ProbeResult("LocalPlayer", V2ProbeStatus.Fail,
                        $"LocalPlayer component pointer vector failed invariant: {compVec.Detail}", details);
                }
        ...
        private static V2ProbeResult ProbeAwakeEntities(SafeMemoryHandle reader)
        {
            ...
            try
            {
                ...
                // Cached collection is comparison evidence only. Verdicts are based on the
                // bounded direct map walk so a stale production cache cannot validate itself.
            ...
            Dictionary<string, string> details)
        {
            ...
            while (pending.Count > 0 && visited < maxNodesVisited && sampled < maxEntitySamples)
            {
                ...
                details[$"{samplePrefix}_Node"] =
                    $"Node=0x{nodeAddress.ToInt64():X}, MapId={mapId}, EntityPtr=0x{entityPtr.ToInt64():X}";
                ...
                if (!CanonicalStructuralInvariants.IsCanonicalPointer(entity.ItemBase.EntityDetailsPtr))
                {
                    details[$"{samplePrefix}_Path"] =
                        $"EntityDetailsPtr=0x{entity.ItemBase.EntityDetailsPtr.ToInt64():X} is non-canonical";
                    continue;
                }
