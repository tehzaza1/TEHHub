    ...
    public static class AutoUpdate
    {
        ...
        private static async Task<bool> DownloadAndInstallUpdateAsync(string version)
        {
            try
            {
                ...
                var tempDir = Path.Combine(Path.GetTempPath(), "TEHhubUpdate");
        ...
        public static void LaunchUpdateAndExit()
        {
            try
            {
                ...
                var launcherPath = Path.Combine(currentDir, "TEHhub.Launcher.exe");
                var tempDir = Path.Combine(Path.GetTempPath(), "TEHhubUpdate");
...
if ($launcherProcess) {{
    ...
    Write-Host 'TEHhub.Launcher has exited.';
}} else {{
    Write-Host 'TEHhub.Launcher process not found, proceeding...';
}}
...
try {{
    ...
    do {{
        ...
        $newLauncherProcess = Get-Process -Name 'TEHhub.Launcher' -ErrorAction SilentlyContinue;
        $timeout++;
    }} while (-not $newLauncherProcess -and $timeout -lt 10);
    if ($newLauncherProcess) {{
        Write-Host 'TEHhub.Launcher restarted successfully.';
    }} else {{
    ...
    Read-Host 'Press Enter to continue';
                ...
                Console.WriteLine("Updating TEHhub. TEHhub.Launcher will restart after update is completed.");
            }
            catch (Exception ex)
            {
